...
- (void)prepareLayout{
    ...
    NSInteger number = [self.delegate sectionNumber];
    if (number == 0) {
        return;
    }
    ...
    for (int section = 0; section < [self.delegate sectionNumber]; section++) {
...
- (void)initDateWithDelegate:(NSInteger) section {
    self.columNumber = [self.delegate columNumberAtSection:section];
    self.columHeightArray = [NSMutableArray arrayWithCapacity:self.columNumber];
    self.sectionInset = [self.delegate sectionInsetAtSection:section];
    self.interSpaceing = [self.delegate interSpaceingAtSection:section];
}

//初始化当前区的第一行的top
- (void)initColumHeightArray:(NSInteger)section {
    double headViewHeight = 0;
    if (self.delegate != nil &&[self.delegate respondsToSelector:@selector(headderSizeForSection:)]) {
        CGSize headSize = [self.delegate headderSizeForSection:section];
        headViewHeight = headSize.height;
    }
    CGFloat marginY = self.sectionHeight + self.sectionInset.top + headViewHeight;
    for(int index = 0; index < self.columNumber;index++){
        self.columHeightArray[index] = @(marginY);
    }
}
- (void)setUpSectionItems:(NSInteger)section {
    CGFloat itemWith = [self.delegate widthAtSection:section];
    //得到section中有多少个item
    NSInteger totalItems = [self.delegate numberAtSection:section];
    ...
    //把得到的当前区的每个item信息记录下来
    [self.attributesSectionArray addObject:[NSArray arrayWithArray:self.attributesArray]];
}
...
- (void)setUpSectionHeader:(NSInteger)section {
    if (self.delegate != nil &&[self.delegate respondsToSelector:@selector(headderSizeForSection:)]) {
        ...
        UICollectionViewLayoutAttributes *atrributes = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind:MyWaterLayoutKindSectionHeader withIndexPath:indexPath];
...
- (void)setUpSectionFooter:(NSInteger)section {
    NSInteger longestIndex = [self longestColumnIndx];
    CGFloat longest = [self.columHeightArray[longestIndex] floatValue];
    CGFloat footHeight;
    if (self.delegate != nil &&[self.delegate respondsToSelector:@selector(footerSizeForSection:)]) {
        CGSize footSize = [self.delegate footerSizeForSection:section];
        if (footSize.height == 0) {
            return;
        }
        footHeight = footSize.height;
        CGRect footFrame = CGRectMake(self.sectionInset.left, 10 + longest, footSize.width, footSize.height + self.sectionInset.bottom);
        NSIndexPath *indexPath = [NSIndexPath indexPathForItem:0 inSection:section];
        UICollectionViewLayoutAttributes *atrributes = [UICollectionViewLayoutAttributes layoutAttributesForSupplementaryViewOfKind: MyWaterLayoutKindSectionFooter withIndexPath:indexPath];
        atrributes.frame = footFrame;
        [self.attributesfooterArray addObject:atrributes];
    }
    self.sectionHeight = longest + footHeight;
}
...
- (NSArray *)layoutAttributesForElementsInRect:(CGRect)rect{
    ...
    for (NSArray *arr in self.attributesSectionArray) {
...
- (CGSize)collectionViewContentSize
{
    ...
    NSInteger currentIndex = [self longestColumnIndx];
...
{
    if (self.columHeightArray.count == 1) {
        return 0;
    }
